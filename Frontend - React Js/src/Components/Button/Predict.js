...
export default function Predict(props) {
  const { select, setSnackopen, setStatus, setRefresh, refresh, setSelect } =
    props;
  ...
  const handleClickOpen = async (e) => {
    ...
    await axios
      .post(
        "http://localhost:8080/BackendServer/predictSend",
    ...
    if (clear_date === "0000-00-00") {
      ...
      await Promise.all(
        res.data.map(async (item) => {
          return axios
            .post(
              "http://localhost:8080/BackendServer/predictUpdate",
              queryString.stringify({
